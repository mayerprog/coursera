...
var result = lib.query(
    friends, //коллекция
    lib.select('name', 'gender', 'email'), //передает результат функции select, функция возвращает объект с именем select и методом action
    lib.filterIn("favoriteFruit", ["Картофель", "Яблоко"]), 
  //передает результат функции filterIn, функция возвращает объект с именем filterIn и методом action
);
